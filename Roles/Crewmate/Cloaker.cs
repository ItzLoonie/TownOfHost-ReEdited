...
public static class Cloaker
{
    ...
    private static OptionItem CloakerCooldown;
    private static OptionItem CloakerDuration;
    ...
    {
        SetupRoleOptions(Id, TabGroup.ImpostorRoles, CustomRoles.Cloaker);
        CloakerCooldown = FloatOptionItem.Create(Id + 2, "CloakerCooldown", new(1f, 999f, 1f), 30f, TabGroup.ImpostorRoles, false).SetParent(CustomRoleSpawnChances[CustomRoles.Cloaker])
            .SetValueFormat(OptionFormat.Seconds);
        CloakerDuration = FloatOptionItem.Create(Id + 4, "CloakerDuration", new(1f, 999f, 1f), 15f, TabGroup.ImpostorRoles, false).SetParent(CustomRoleSpawnChances[CustomRoles.Cloaker])
            .SetValueFormat(OptionFormat.Seconds);
    }
    ...
    private static void SendRPC(PlayerControl pc)
    {
        ...
        MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetCloakerTimer, SendOption.Reliable, pc.GetClientId());
    ...
    public static void OnFixedUpdate(PlayerControl player)
    {
        ...
        if (lastTime.TryGetValue(player.PlayerId, out var time) && time + (long)CloakerCooldown.GetFloat() < now)
        {
            lastTime.Remove(player.PlayerId);
            if (!player.IsModClient()) player.Notify(GetString("CloakerCanVent"));
            SendRPC(player);
        }
        ...
        if (lastFixedTime != now)
        {
            ...
            foreach (var it in InvisTime)
            {
                ...
                if (pc == null) continue;
                var remainTime = it.Value + (long)CloakerDuration.GetFloat() - now;
                if (remainTime < 0)
                {
                    ...
                    NameNotifyManager.Notify(pc, GetString("CloakerInvisStateOut"));
                    SendRPC(pc);
                    continue;
                }
                else if (remainTime <= 10)
                {
                    if (!pc.IsModClient()) pc.Notify(string.Format(GetString("CloakerInvisStateCountdown"), remainTime));
                }
    ...
    public static void OnCoEnterVent(PlayerPhysics __instance, int ventId)
    {
        ...
        new LateTask(() =>
        {
            if (CanGoInvis(pc.PlayerId))
            {
                ...
                SendRPC(pc);
                NameNotifyManager.Notify(pc, GetString("CloakerInvisState"), CloakerDuration.GetFloat());
            }
            else
            {
                __instance.myPlayer.MyPhysics.RpcBootFromVent(ventId);
                NameNotifyManager.Notify(pc, GetString("CloakerInvisInCooldown"));
            }
        }, 0.5f, "Cloaker Vent");
    }
    public static void OnEnterVent(PlayerControl pc, Vent vent)
    {
        if (!pc.Is(CustomRoles.Cloaker) || !IsInvis(pc.PlayerId)) return;
    ...
    public static string GetHudText(PlayerControl pc)
    {
        ...
        {
            var remainTime = InvisTime[pc.PlayerId] + (long)CloakerDuration.GetFloat() - Utils.GetTimeStamp();
            str.Append(string.Format(GetString("CloakerInvisStateCountdown"), remainTime));
        }
        else if (lastTime.TryGetValue(pc.PlayerId, out var time))
        {
            var cooldown = time + (long)CloakerCooldown.GetFloat() - Utils.GetTimeStamp();
            str.Append(string.Format(GetString("CloakerInvisCooldownRemain"), cooldown));
        }
        else
        {
            str.Append(GetString("CloakerCanVent"));
        }
        return str.ToString();
    }
