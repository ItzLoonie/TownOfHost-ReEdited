    ...
    public static void Postfix(PlayerControl __instance)
    {
                ...
                if (GameStates.IsInTask && Main.PuppeteerList.ContainsKey(player.PlayerId))
                {
                    ...
                    else
                    {
                        ...
                        foreach (var target in Main.AllAlivePlayerControls)
                        {
                            ...
                            {
                                if (target.PlayerId != player.PlayerId && !target.Is(CustomRoleTypes.Impostor) && !target.Is(CustomRoles.Glitch) && !target.Is(CustomRoles.Pestilence))
                                {
        ...
        if (RoleText != null && __instance != null && !lowLoad)
        {
            ...
            if (GameStates.IsInGame)
            {
                ...
                else if (__instance.Is(CustomRoles.Sidekick) && (PlayerControl.LocalPlayer.Is(CustomRoles.Jackal) || PlayerControl.LocalPlayer.Is(CustomRoles.Recruit))) RoleText.enabled = true;
                else if (__instance.Is(CustomRoles.Recruit) && (PlayerControl.LocalPlayer.Is(CustomRoles.Jackal) || PlayerControl.LocalPlayer.Is(CustomRoles.Sidekick))) RoleText.enabled = true;
